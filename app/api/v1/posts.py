...
async def vote_post(post_id: str, payload: dict, me: UserIdDep, posts: PostsDep):
    ...
    if value not in (-1, 1):
        raise HTTPException(status_code=400, detail="value debe ser -1 o 1")
...
async def delete_post(post_id: str, me: UserIdDep, posts: PostsDep):
    if not me:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="No autenticado")
    await posts.delete(post_id=post_id, user_id=me)
    return
